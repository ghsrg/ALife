...
У MVP можна почати з простого пропорційного розподілу з невеликим шумом.
...
Energy не передається як речовина в середовище.

Але під час поділу внутрішній стан батьківської клітини розділяється між дочірніми клітинами.
...
Для MVP можна почати з копіювання одного Genome з шансом мутації.
...
Точні пороги визначаються MVP-конфігурацією.
...
# Мінімальний Lifecycle для MVP

Для MVP достатньо таких станів:
...
Result:
  division fails
  Energy is partially consumed
  Boundary is damaged
  cell enters stressed state
```

Або, якщо модель дозволяє частковий поділ:

```text
Result:
  one weak daughter cell
  one damaged parent-like cell
  weak daughter lacks enough Boundary
  weak daughter dies after several Tick
```
...
```text
ADR-000X: Cell Death as Structural Failure
ADR-000X: Dead Cells Remain Physical Objects
ADR-000X: Division Can Produce Non-Viable Offspring
ADR-000X: Aging as Accumulated Degradation
```
...
## Minimal viability formula

Потрібно визначити точну MVP-формулу мінімальної життєздатності.

Кандидати:

```text
boundary_integrity >= min_boundary
critical_materials >= min_materials
genome_functionality >= min_genome
maintenance_possible == true
```

## Division model

Потрібно визначити MVP-модель поділу:

* завжди на дві клітини;
* з випадковим шумом розподілу;
* з possibility failed division;
* з progress-based division preparation.
...
Потрібно визначити, яку частину epigenetic_state передавати дочірнім клітинам.

## Genome fragments after death

Потрібно визначити, чи Genome fragments можуть існувати в середовищі в MVP, чи це буде відкладено до HGT-моделі.
