---
tags:
  - alife
  - canon
  - area/biology
---

# feasibility.md

> **Feasibility Check — єдиний contract перевірки можливості planned actions**

---

# Призначення

`feasibility.md` визначає, як planned action перевіряється перед виконанням.

Feasibility Check не виконує дію. Він відповідає, чи дія може бути виконана, і чому вона відхилена.

---

# Scope

Feasibility Check застосовується до active planned actions:

```text
registered planned actions
registered controlled reactions
```

Canonical process/action ids are defined in `biology/action-process-registry.md`.

Mandatory existence costs обробляються перед planned actions і описані в `world/energy.md`.

---

# FeasibilityResult

```text
FeasibilityResult
├── status: allowed | rejected
├── action_id
├── required_energy
├── required_resources
├── required_materials
├── required_space
├── blocked_by
├── failure_reasons
└── diagnostics
```

`allowed` означає, що дія може перейти до ProcessExecution.

`rejected` означає, що дія не виконується, не має часткового результату і не збільшує ProcessProgress.

Feasibility Check for planned actions must use post-mandatory state:

```text
available_for_planned_actions =
  committed_state_after_mandatory_costs
```

---

# Failure Reason Taxonomy

Базові reasons:

- `insufficient_energy`
- `insufficient_resource`
- `insufficient_material`
- `insufficient_space`
- `boundary_damaged`
- `boundary_blocked`
- `boundary_requires_active_transport`
- `material_incompatible`
- `physics_blocked`
- `joint_blocked`
- `lifecycle_state_blocked`
- `invalid_genome_request`
- `conflict_unresolved`

Failure reason має бути явним, щоб debug/observer могли пояснити, чому клітина не живе, не росте або не ділиться.

---

# No World Mutation

Feasibility Check не змінює:

- Resources;
- Materials;
- Energy;
- Physics;
- Lifecycle;
- Genome;
- Joint;
- ProcessProgress.

Mandatory costs are resolved before Feasibility for planned actions. Feasibility must not approve an action set using pre-mandatory state.

---

# Rejected Action Rule

Rejected action:

- не списує cost;
- не створює output;
- не має partial final product;
- не збільшує ProcessProgress;
- може бути записана в diagnostics/trace.

---

# Process Contract

Усі active process execution повинні проходити через цей contract.

Process не може мати приховану власну логіку "можна / не можна", яка обходить Feasibility Check.

Feasibility Check accepts only registered planned actions or controlled reactions.

---

# Future Reservation Model

Resource/Energy reservation може бути додана пізніше.

У базовій моделі Feasibility Check дає diagnostics і status, але не резервує state наперед.

---

# Rules

## Rule 1. Feasibility is read-only

Feasibility Check не змінює world state.

## Rule 2. Rejection is explicit

Rejected action має explicit failure reasons.

## Rule 3. No partial result

Rejected planned action не має часткового результату.

## Rule 4. Genome cannot bypass feasibility

Genome output є regulatory intent, а не дозвіл на зміну світу.

## Rule 5. Feasibility uses post-mandatory state

Planned actions cannot consume Energy or Resources required for mandatory existence.

## Rule 6. Boundary gates resource exchange

Feasibility for uptake checks:

- Resource exists locally.
- Boundary Material has a permeability rule for Resource id or class.
- The rule is `passive` or `active_required`.
- For `active_required`, the Cell has matching Material capability, Energy and process capacity.
- Internal capacity is available.
- Boundary integrity is not below the critical threshold for controlled uptake.

Feasibility for export checks:

- Resource exists inside the Cell.
- Boundary Material allows export or leakage for Resource id/class.
- Passive leakage or active export process is available.
- Active export has matching channel/capability/Energy when required.

If no matching rule exists, `default_permeability: blocked` applies.

---

# Semantic Links

- gates execution of: [[docs/biology/processes|Processes]]
- checks availability of: [[docs/world/energy|Energy]]
- checks availability of: [[docs/world/resources|Resources]]
- checks capabilities from: [[docs/world/materials|Materials]]
- constrains outputs from: [[docs/biology/genome|Genome]]

# Пов'язані документи

- `biology/processes.md`
- `biology/process-progress.md`
- `genetics/regulatory-interface.md`
- `world/energy.md`
- `world/tick-semantics.md`
