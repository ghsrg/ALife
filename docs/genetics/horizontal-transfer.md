...
Для MVP можна почати з простого джерела:
...
У спрощеному MVP fragment може бути абстрактним локальним object.
...
Для MVP достатньо залишити це як можливість.
...
Це теж можна відкласти до post-MVP.

---

# HGT через середовище

Найпростіший сценарій для MVP або early post-MVP:
...
Для MVP HGT можна не реалізовувати.
...
```text
ADR-000X: Horizontal Transfer Requires Physical Genetic Fragments
ADR-000X: HGT Is Not Inheritance and Not Mutation
ADR-000X: HGT Compatibility Does Not Use Species ID
ADR-000X: Virus-like Behavior Is Emergent, Not Hardcoded
```
...
Потрібно вирішити, чи HGT входить у MVP, чи тільки post-MVP.
