...
Формула не є обов'язковою для MVP.
...
Для MVP саме `edge_weight_shift` може бути основним механізмом.
...
Для MVP фрагменти можна відкласти, але не блокувати архітектурно.
...
* немає заборонених self-reference, якщо вони заборонені MVP;
...
# Mutation Operators for MVP

Для MVP достатньо мінімального набору операторів:
...
# Recommended MVP Mutation Order
...
Для MVP краще починати з малих мутацій і рідкісних структурних змін.
...
```text
ADR-000X: Mutation Is Random, Not Directed
ADR-000X: Technical Validity Does Not Mean Biological Viability
ADR-000X: Learning Is Not Genome Mutation
ADR-000X: MVP Mutation Starts with Parametric Changes
```
...
Потрібно остаточно вибрати, які оператори включити в MVP.
...
Потрібно вирішити, чи structural mutations будуть у MVP, чи тільки після стабілізації базової еволюції.
...
Потрібно визначити, чи Radiation, Heat і reactive Resources впливають на mutation_rate у MVP.
