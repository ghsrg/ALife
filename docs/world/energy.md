...
Якщо Energy недостатньо, дія не виконується або виконується частково, якщо це дозволено правилами відповідної механіки.
...
- перейти в локальне Heat;
- підвищити температуру клітини;
...
Для базової моделі достатньо локального Heat у клітині та передачі через контакт або Joint.
...
Heat є фізичним станом або локальним наслідком втрат.
...
У базовій моделі Heat не обов'язково має ставати глобальним полем.
...
Надлишкова Energy може перейти в Heat, локальні втрати або реакції.
...
Потрібен окремий ADR для рішення:

```text
Energy is local cell buffer, not Resource and not Material.
```
...
- локальним станом клітини;
- частиною `Field`;
- частиною `physics.md`;
- або спрощеним побічним ефектом без окремого поширення.

## Partial action execution

Потрібно визначити, чи дія може виконатися частково, якщо Energy недостатньо.
